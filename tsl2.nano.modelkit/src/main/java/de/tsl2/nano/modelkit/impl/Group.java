...
/**
 * Sorting Group, selecting its items through a selector and providing a
...
public class Group<T> extends AbstractIdentified implements Comparator<T>, Selectable<T> {
    ...
    public Group(String name, int limit, String groupFunction, int step, String selectorFact,
            String... comparatorNames) {
        this(name, limit, groupFunction, step, selectorFact, false, comparatorNames);
    }

    /**
     * creates a new Group. a group sorts elements through comparators, splits
     * subgroups through given groupFunction and limit,
     * and inserts group items on higher positions given by
     * step/limit/insertionLimitFallDown.
     *
     * @param name
     *                               unique name
     * @param limit
     *                               primarily used by group splitting to a max of
     *                               given limit. secondarily used by insertion to
     *                               indicate limit of
     *                               equal insertions
     * @param groupFunction
     *                               defines group splitting and is used on
     *                               insertion limit
     * @param step
     *                               used by insertion for insertion position - and
     *                               shifting index on reached limit
     * @param selectorFact
     *                               defines the filter for this group
     * @param insertionLimitFallDown
     *                               used by insertion to indicate if an insertion
     *                               item falls to end of group on reached limit
     * @param comparatorNames
     *                               selectable comparators to be used by group
     *                               sorting
     */
    public Group(String name, int limit, String groupFunction, int step, String selectorFact,
            boolean insertionLimitFallDown,
            String... comparatorNames) {
        super(name);
        this.limit = limit;
        ...
        this.comparatorNames = Arrays.asList(comparatorNames);
    }
    ...
    public void tagNames(String parent) {
        ...
        groupFunction = tag(parent, groupFunction);
        selectorFact = tag(parent, selectorFact);
        tag(parent, comparatorNames);
    }
    ...
    /**
     * main action for group. does the sorting wich comparators, splitting subgroups
     * and insertions.
     */
    ...
    /**
     * the group limit will be accessed on the given group by expression. E.g. if
     * the groupByExpression targets a field, a hashmap on all groups pointing to
     * list of items with that groupName will be
     * splitted after group limit (f.e. limit=3) and the rest will be shift to the
     * bottom of the list.
     */
    List<T> groupByLimit(List<T> groupItems) {
        return splitGroups(groupBy(groupItems, (Function<T, Object>) get(groupFunction, Func.class).getFunction()),
                limit);
    }
    ...
    private List<T> splitGroups(Map<String, List<T>> grouped, int groupLimit) {
        ...
        for (var entry : grouped.entrySet()) {
            ...
            List<T> firstElements = groupItems.subList(0, Math.min(groupLimit, groupItems.size()));
            items.addAll(firstElements);
            firstElements.clear();
        }
        if (iterationNeeded) {
            items.addAll(splitGroups(grouped, groupLimit));
        }
        return items;
    }
     ...
     *
     * @param allItems
     *                         origin list to position the items of current group
     *                         upper
     * @param insertedBefore
     *                         if a higher group has call insertion() for its items
     *                         before, don't overwrite them. normally the previous
     *                         group.
     * @param lowerGroupToStop
     *                         do insertions until items of given group are reached.
     *                         normally the following group.
