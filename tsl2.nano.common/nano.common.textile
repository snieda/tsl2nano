...
* Different Implementations of "IAction":src/main/de.tsl2.nano/action/IAction.java to be describors for creating GUI-Buttons
...
The AppLoader helps you defining a main class with arguments, help and an own classloader. The Environment is a singelton holding a pointer to a directory to store all configurations of an application. The environment provides all properties and services of the application. It provides rersource bundles, the classloader etc. in a simplified way.It may be used as a central implementation helper.
