...
* swartifex.serviceaccess
* swartifex.directaccess

It is possible to use a ejb container in an application server, but the default is set to use jpa directly on the client (using _swartifex.directaccess_).
...
If you change sources of this plugin, you should start ant script _eclipse-tsl2.nano.h5.xml_ with target _swartifex.eclipse.h5.export_ and after that the 'deploy' target of your main project.
...
* Use Xml-Configuration for BeanDefinitions
* Html-Table: Sorting-Header, Style
...
* default attribute names without @id field
