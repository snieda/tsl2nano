...
Generic html5 gui implementation providing a model driven access (MDA), bound to __tsl2.nano.commons__ bean-package and __tsl2.nano.serviceaccess__ and it's __GenericService__ using __javax.persistence__. It is possible to design a class- or database-model, exporting it and to start a build process resulting in a complete html application, providing a comfortable way to edit all entities and their relations.
...
* navigation can be configurable workflow - or simply an entity browser 
...
h4. Model Driven Architecture (MDA))
