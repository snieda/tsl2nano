...
<meta-tag>
crud, grud, crud2gui, crud2html, bean2html, entity2html, bean2gui, entity2gui, jpa2gui, jpa2html, jpa persistence provider, openjpa, hibernate, datanucleus, eclipselink, data-editor, data-sheet, entity browser, jpa2, full stack framework, orm, o/r mapper
</meta-tag>
...
* as a test base to check your model on different jpa persistence providers like *hibernate*, *elcipselink*, *openjpa* and *datanucleus*. you can switch between them with three mouseclicks.
...
* [OpenJPA    ]: http://openjpa.apache.org: _org.apache.openjpa.persistence.PersistenceProviderImpl_
...
* IConnector: defines the login/authentification. On default, the NanoH5 main instance is providing that through an Persistence instance.
...
* jpa o/r mapper (like hibernate, toplink, eclipselink, openjpa or ormlite)
...
Each bean and attribute can be presented by and created by a text expression. On beans, this expression (called _ValueExpresion_) should contain at least one bean-attribute (perhaps the id, but if another attribute is more readable but unique, you would prefer this). These _ValueExpression_ will be used to present a bean in a list, or if it used as relation in another bean. The creation through a string works only, if the bean provides a default-constructor and the attributes in the expression have setter-methods.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        ((AttributeDefinition)b.getAttribute("organisation")).changeHandler().addListener(new WebSocketDependencyListener() {
...
As you can see, all messages that will change attribute properties
...
* ant wont work, so
** as hibernate doesn't work on android too, *hibernate-tools* as bean-jar generator is not usable.
...
*** use prefix '!' to define, that no environment-prefix will be added. E.g. '!mybeans.jar'.
...
* NetworkClassLoader should recognize resources (e.g. for loggers) to avoid reloading from network every time.
...
* problems on event 'onchange' with value = 'function() {...}' on attachments
