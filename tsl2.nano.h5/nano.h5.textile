...
h2. Security, Authorization and Permissions

Permissions are set after connecting to the datasource through the persistence-unit. The permissions define activation of buttons and showing of fields.

The application class _NanoH5_ has a method _createSubject()_ that defines a subject with a user-principal and it's roles, defined by permissions. If a file _permissions.xml_ is found, it will be used to fill the subject - if not, an admin-role with a wildcard-permission will permit anything!

Permissions contain a name - perhaps ended by a wildcard - and comma separated actions (a wildcard is possible, too.
