...
import static de.tsl2.nano.bean.def.BeanPresentationHelper.PROP_DESCRIPTION;
import static de.tsl2.nano.bean.def.BeanPresentationHelper.PROP_ENABLER;
import static de.tsl2.nano.bean.def.BeanPresentationHelper.PROP_LABEL;
...
import static de.tsl2.nano.bean.def.BeanPresentationHelper.PROP_VALUE;
...
public class PersistenceUI {
    ...
    public static Bean<Persistence> createPersistenceUI(final Persistence persistence,
        ...
        if (login.isDefault()) {
            ...
            if (ENV.isModeAdmin() && !NanoH5ExternalBackend.isBackendDefined()) {
                addAdminFeatures(persistence, login, helper);
        ...
        if (NanoH5ExternalBackend.isBackendDefined()) {
            helper.change(PROP_LABEL, "Backend Url", "connectionUrl");
            helper.change(PROP_DESCRIPTION, "Backend Url", "connectionUrl");
            if (ENV.isModeAdmin()) {
                helper.change(PROP_VISIBLE, true, "connectionUrl");                
                helper.change(PROP_VALUE, NanoH5ExternalBackend.definedBackend(), "connectionUrl");                
                String serviceUrl = ENV.get("service.url", "http://localhost:8067");
                ((IIPresentable) login.getAttribute("connectionUrl").getPresentation())
                    .setItemList(Arrays.asList(
                        serviceUrl + "/rest/party/id/1",
                        serviceUrl + "rest/backend"));
            }
        }
...
//        ((Map)login.getPresentable().getLayoutConstraints()).put("style", "opacity: 0.9;");
    ...
    private static void addAdminFeatures(final Persistence persistence, final Bean<Persistence> login, BeanPresentationHelper<Persistence> helper) {
        /*
