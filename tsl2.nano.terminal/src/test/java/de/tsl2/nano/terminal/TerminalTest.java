...
import de.tsl2.nano.terminal.TerminalAdmin;
...
import de.tsl2.nano.terminal.item.MainAction;
...
//    	ENVTestPreparation.tearDown();
  ...
  public void testSIShellTools() throws Exception {
      ...
      xml.add(new Sequence(new de.tsl2.nano.terminal.item.Command("sequential echo command", "echo"),
          new XPathSelector(
      ...
      vnet.add(new FileSelector("configuration", null, ".*xml", "${user.dir}"));
      vnet.add(impl);
      vnet.add(new MainAction(NetCommunicator.class, "configuration", "implementation=${implementation}"));
      root.add(vnet);
...
//
      ...
      shell.add(new de.tsl2.nano.terminal.item.Command("command", "cmd /C"));
      root.add(shell);
