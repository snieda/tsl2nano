...
This project provides implementations to access services through a "servicefactory":src/main/de.tsl2.nano/serviceaccess/ServiceFactory.java. The JAAS is used to do authentification and authorization (see: "here":src/main/de.tsl2.nano/serviceaccess/aas/module/AbstractLoginModule.java). A feature-mechanism provides the possibility to do optional calls to feature-implementations. 
...
h3. Using on different Application Servers

This plugins contains some jboss specific api jars:
...
* "FileSystem-Connector":fsconnector.html simple filesystem connector implementation. a simple "FileService":src/main/de.tsl2.nano/service/util/FileServiceBean.java simplifies the use. have a look at the junit-test "FileServiceTest":src/test/de/idvag/serviceaccess/test/FileServiceTest.java to see some use-cases.c
...
To see, how to use the the services in the simpliest way, have a look at the test class "ServiceAccessTest":src/test/de/idvag/serviceaccess/test/ServiceAccessTest.java.
...
public class LoginModule extends AbstractLoginModule {
    protected boolean authenticate() throws FailedLoginException {
        ...
        if (b == null)
            throw new FailedLoginException(Messages.getString("swartifex.login.error.userorpasswd"));
...
"4.14 Optional Feature Integration on Runtime":..\swartifex\doc\readme.txt.html#4.14
...
<pre>
	...
	<enterprise-beans>
		<session>
			...
			<mapped-name>kion/IGenericService</mapped-name>
			<ejb-class>de.tsl2.nano.service.util.GenericServiceBean</ejb-class>
			<session-type>Stateless</session-type>
			<ejb-local-ref>
				<ejb-ref-name>kion/IGenericLocalService</ejb-ref-name>
			...
			<!--
				...
				<persistence-context-ref-name>persistence/kionPersistenceUnit</persistence-context-ref-name>
				<persistence-unit-name>../kion-services.jar#kionPersistenceUnit</persistence-unit-name>
				</persistence-context-ref>
			-->
		</session>
		...
...
<pre>
	<session>
		...
		<mapped-name>kion/IJobScheduleService</mapped-name>
		<ejb-class>de.tsl2.nano.service.schedule.JobScheduleServiceBean</ejb-class>
		<session-type>Singleton</session-type>
		<ejb-local-ref>
			<ejb-ref-name>kion/IJobScheduleLocalService</ejb-ref-name>
			<local>de.tsl2.nano.service.schedule.IJobScheduleLocalService</local>
		</ejb-local-ref>
		<persistence-unit-ref>
			<persistence-unit-ref-name>../kion-services.jar#kionPersistenceUnit</persistence-unit-ref-name>
		</persistence-unit-ref>
	</session>
</pre>
...
If you want the swartifex framework to work on your bean like a normal entity, create the two named queries 'findAll' and 'findById'.
...
* _eclipse-tsl2.nano.serviceaccess.xml_ with target _swartifex.eclipse.serviceaccess.export_ (Please be sure to use the same jre-runtime as your eclipse-workspace - otherwise nothing will be done!).
* the 'deploy' target of your main project.
* perhaps an additional script to distribute the new libraray to a project-specific library-plugin. in _kion2plus_ it is _kion.swartifex.plugins_. In _kion2plus_ you run _kion/distribute-swartifex-plugins.xml_ with target _copy.swartifex.plugins_.
