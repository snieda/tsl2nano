...
* _eclipse-tsl2.nano.serviceaccess.xml_ with target _tsl2nano.eclipse.serviceaccess.export_ (Please be sure to use the same jre-runtime as your eclipse-workspace - otherwise nothing will be done!).
* the 'deploy' target of your main project.
* perhaps an additional script to distribute the new libraray to a project-specific library-plugin. in _kion2plus_ it is _tsl2.nano.plugins_. In _kion2plus_ you run _kion/distribute-tsl2nano-plugins.xml_ with target _copy.tsl2nano.plugins_.
